...
type Task struct {
	...
	RetryTimes          int    `gorm:"column:retry_times;type:tinyint(4);not null;default:3" json:"retry_times"`              // 重试次数
	RetryInterval       int32  `gorm:"column:retry_interval;type:int(11);not null;default:60" json:"retry_interval"`          // 重试间隔(单位:秒)
	NotifyStatus        int    `gorm:"column:notify_status;type:tinyint(4);not null" json:"notify_status"`                    // 执行结束是否通知 1:不通知 2:失败通知 3:结束通知 4:结果关键字匹配通知
	NotifyType          int    `gorm:"column:notify_type;type:tinyint(4);not null;default:1" json:"notify_type"`              // 通知类型 1:邮件 2:webhook
	...
	Status              int    `gorm:"column:status;type:tinyint(4);not null;default:1" json:"status"`                        // 状态1:启用  2:停用
